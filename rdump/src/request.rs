...
use crate::{
    commands::search::perform_search_internal, SearchDiagnostic, SearchOptions, SearchResult,
};
    ...
    F: FnMut(&ProgressEvent),
{
    ...
    progress(&ProgressEvent::Started {
        session_id: "sync-request".to_string(),
        query: request.query.clone(),
        effective_query: explanation.effective_query.clone(),
        root: root.clone(),
        queue_wait_millis: 0,
    });
    ...
    let stats = match output {
        OutputMode::Paths => {
            let raw_report = perform_search_internal(&request.query, &options)?;
            diagnostics.extend(raw_report.diagnostics.iter().map(map_diagnostic));
            let core_stats = raw_report.stats.clone();
            let mut emitted = 0usize;

            emit_engine_phase_progress(&mut progress, &core_stats, "sync-request");

            progress(&ProgressEvent::Phase {
                session_id: "sync-request".to_string(),
                name: "materialize".to_string(),
                completed_items: 0,
                total_items: Some(raw_report.results.len().saturating_sub(request.offset)),
            });

            for raw in raw_report.results.into_iter().skip(request.offset) {
                if results.len() >= limits.max_results {
                    truncated = true;
                    truncation_reason = Some("max_results".to_string());
                    next_offset = Some(request.offset + emitted);
                    break;
                }

                let shaped = SearchItem::Path {
                    path: render_contract_path(
                        &crate::FileIdentity {
                        ...
                        request.path_display,
                    ),
                    file: map_file_identity(&crate::FileIdentity {
                        display_path: raw.display_path.clone(),
                        resolved_path: raw.resolved_path.clone(),
                        root_relative_path: raw.root_relative_path.clone(),
                        resolution: raw.resolution,
                    ...
                    result_kind: ContractResultKind::WholeFile,
                    item_truncated: false,
                };
                let next_bytes = returned_bytes.saturating_add(measure_serialized_bytes(&shaped));
                ...
                progress(&ProgressEvent::Result {
                    session_id: "sync-request".to_string(),
                    path: item_path(results.last().unwrap()).to_string(),
        ...
        _ => {
            let iter = crate::search_iter(&request.query, options)?;
            diagnostics.extend(iter.diagnostics().iter().map(map_diagnostic));
            let core_stats = iter.stats().clone();
            let mut emitted = 0usize;
            ...
            for (index, item) in iter.enumerate() {
                ...
                let result = match item {
                    Ok(result) => result,
                    Err(err) => {
                        handle_error(&mut errors, &limits, request.error_mode, err)?;
                        continue;
                    }
                };

                diagnostics.extend(result.diagnostics.iter().map(map_diagnostic));
                ...
                let (shaped, match_count) = build_item(
                    output,
                    &result,
                    context_lines,
                    request.snippet_mode,
                    &limits,
                    request.path_display,
                    request.line_endings.unwrap_or(LineEndingMode::Preserve),
                    request.include_match_text,
                )?;
                let next_bytes = returned_bytes.saturating_add(measure_serialized_bytes(&shaped));
                if next_bytes > limits.max_total_bytes {
                    truncated = true;
    ...
    let stats = ContractStats {
        ...
        prefiltered_files: stats.prefiltered_files,
        evaluated_files: stats.evaluated_files,
        matched_files: stats.matched_files,
        matched_ranges: stats.matched_ranges,
        hidden_skipped: stats.hidden_skipped,
        ignore_skipped: stats.ignore_skipped,
        max_depth_skipped: stats.max_depth_skipped,
        unreadable_entries: stats.unreadable_entries,
        root_boundary_excluded: stats.root_boundary_excluded,
        suppressed_too_large,
        suppressed_binary,
        suppressed_secret_like,
        diagnostics: diagnostics.len(),
        walk_millis: stats.walk_millis,
        prefilter_millis: stats.prefilter_millis,
        evaluate_millis: stats.evaluate_millis,
        materialize_millis: stats.materialize_millis,
        semantic_parse_failures: stats.semantic_parse_failures,
        semantic_budget_exhaustions: stats.semantic_budget_exhaustions,
        query_cache_hits: stats.query_cache_hits,
        query_cache_misses: stats.query_cache_misses,
        tree_cache_hits: stats.tree_cache_hits,
        tree_cache_misses: stats.tree_cache_misses,
        semaphore_wait_millis: stats.semaphore_wait_millis,
        semantic_parse_failures_by_language: stats.semantic_parse_failures_by_language,
        directory_hotspots: stats.directory_hotspots,
    };
    ...
    Ok(SearchResponse {
        ...
        page_size: Some(limits.max_results),
    })
}
