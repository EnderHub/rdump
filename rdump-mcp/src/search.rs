...
use rdump::request::{execute_search_request_with_progress, format_search_text};
...
{
    execute_search_request_with_progress(&params, |event| {
        progress(event);
        if cancellation.is_cancelled() {
            progress(&ProgressEvent::Finished {
                session_id: "mcp-search".to_string(),
                returned_files: 0,
                returned_matches: 0,
                truncated: true,
            });
