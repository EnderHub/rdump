...
use crate::search::{build_search_request, format_search_response_text};
use crate::types::{
    DescribeLanguageArgs, ExplainQueryArgs, SearchArgs, ValidateQueryArgs, ValidateQueryResponse,
};
...
struct CachedSearchSession {
    pager: rdump::request::SearchRequestPager,
    created_at: Instant,
...
impl RdumpServer {
        ...
        if let Some(token) = args.continuation_token.as_deref() {
            match parse_session_token(token) {
                Ok((session_id, offset)) => {
                    return self
                        .search_session_page(&session_id, offset, request_id)
                        .await
                }
                Err(message) => {
        ...
        let response = tokio::task::spawn_blocking(
            move || -> McpResult<(rdump::contracts::SearchResponse, Option<CachedSearchSession>)> {
                ...
                let response = pager
                    ...
                    .map_err(|err| McpError::tool_execution_failed("search", err.to_string()))?;
                let session = if response.next_offset.is_some() && !pager.is_finished() {
                    let now = Instant::now();
    ...
    async fn search_session_page(
        ...
        let response = tokio::task::spawn_blocking({
            let search_sessions = self.search_sessions.clone();
            let session_ttl = self.session_ttl;
            let session_metrics = self.session_metrics.clone();
            let session_id = session_id.to_string();
            move || -> McpResult<rdump::contracts::SearchResponse> {
                let _permit = permit;
                let mut guard = search_sessions
                    .lock()
                    .expect("search session lock poisoned");
                prune_expired_sessions_locked(&mut guard, session_ttl, &session_metrics);
                let Some(cached) = guard.get_mut(&session_id) else {
                    return Err(McpError::invalid_request(format!(
                        "Unknown continuation session `{session_id}`"
                    )));
                };
                if cached.last_accessed.elapsed() > session_ttl {
                    guard.remove(&session_id);
                    session_metrics.evicted_ttl.fetch_add(1, Ordering::Relaxed);
                    return Err(McpError::invalid_request(format!(
                        "Continuation session `{session_id}` expired after {}s of inactivity",
                        session_ttl.as_secs()
                    )));
                }
                if offset != cached.pager.current_offset() {
                    return Err(McpError::invalid_request(format!(
                        "Continuation offset `{offset}` does not match the live session cursor `{}`",
                        cached.pager.current_offset()
                    )));
                }
                ...
                let mut response = cached
                    ...
                    .map_err(|err| McpError::tool_execution_failed("search", err.to_string()))?;
                if response.next_offset.is_none() || cached.pager.is_finished() {
                    guard.remove(&session_id);
                } else {
                    response.continuation_token = response
                        .next_offset
                        .map(|next| format_session_token(&session_id, next));
                }
                Ok(response)
            }
        })
        ...
        response.stats.semaphore_wait_millis = queue_wait_started.elapsed().as_millis() as u64;
        self.maybe_attach_queue_overload_diagnostic(&mut response);
        let text = format_search_response_text(&response);
        tool_result(&response, text)
    }
    ...
    fn session_cache_text(&self) -> String {
        ...
        let retained_bytes: usize = guard
            .values()
            .map(|session| session.pager.estimated_state_bytes())
            .sum();
