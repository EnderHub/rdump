...
**Basic Tests:**
1. - [x] `test_search_async_basic` - passes
2. - [x] `test_search_all_async_basic` - passes
3. - [x] `test_search_async_empty_results` - passes
4. - [x] `test_search_all_async_empty_results` - passes

**Stream Behavior:**
5. - [x] `test_search_async_early_termination` - passes
6. - [x] `test_search_async_drop_stream` - passes
7. - [x] `test_search_async_concurrent_processing` - passes

**Error Handling:**
8. - [x] `test_search_async_error_propagation` - passes
9. - [x] `test_search_all_async_error_propagation` - passes

**All Tests Pass:**
10. - [x] `cargo test --features async --test async_api`

## Tasks / Subtasks

- [x] Create test file `tests/async_api.rs`
- [x] Add feature gate `#![cfg(feature = "async")]`
- [x] Write basic functionality tests (AC: 1, 2, 3, 4)
  - [x] Basic streaming
  - [x] Collection convenience
  - [x] Empty results
- [x] Write stream behavior tests (AC: 5, 6, 7)
  - [x] Early termination with `.take()`
  - [x] Stream drop stops producer
  - [x] Concurrent stream processing
- [x] Write error handling tests (AC: 8, 9)
- [x] Verify all tests pass (AC: 10)

## Dev Agent Record
- Agent Model Used: GPT-5 (Codex CLI)
- Debug Log References: N/A
- Completion Notes:
  - Expanded `rdump/tests/async_api.rs` with async integration coverage: basic streaming, collection, empty results, early termination, drop semantics, concurrent processing, and error propagation for both streaming and collect-all helpers.
  - Verified via `cargo test --features async --offline --test async_api`.
- File List:
...
| 2025-11-21 | 0.2 | Added async integration tests for streaming and collect APIs | Dev Agent |
