...
### Version 0.2.0 - Enhanced Usability

**Target:** Q1 2025
**Theme:** Make rdump easier to learn and use interactively
...
**Description:** REPL for exploratory queries with history and auto-completion

**User Story:** As a developer new to rdump, I want an interactive mode where I can experiment with queries and see results immediately, so that I can learn the query language faster.

**Detailed Requirements:**
- Start with `rdump --interactive` or `rdump -i`
- Persistent history across sessions (`~/.rdump_history`)
- Tab completion for:
  - Predicate names (`ext:`, `func:`, etc.)
  - Recent values
  - File paths in current directory
- Live syntax highlighting in the prompt
- Result preview as you type (optional, with `--live`)
- Multi-line queries with `\` continuation

**Implementation Approach:**
```rust
// Use rustyline for readline with completions
...
fn interactive_mode() -> Result<()> {
    ...
    rl.set_helper(Some(RdumpCompleter::new()));
    rl.load_history("~/.rdump_history").ok();

    loop {
        match rl.readline("rdump> ") {
            Ok(line) => {
...
- [ ] Tab completion works for predicates
- [ ] Ctrl+C cancels current query, doesn't exit
- [ ] Ctrl+D exits cleanly

#### 2. Query Builder

**Description:** Guided query construction for users unfamiliar with RQL

**User Story:** As a developer who doesn't remember all predicate names, I want a guided query builder that suggests predicates and validates my input, so that I can construct queries without memorizing syntax.

**Wizard Flow:**
```
$ rdump --wizard

Welcome to rdump Query Builder!

Step 1: What type of files? (ext, name, path)
> ext
Enter extension: rs

Current query: ext:rs

Step 2: Add content filter? (y/n)
> y
Choose: (1) contains  (2) matches - regex
> 1
Enter text: async fn

Current query: ext:rs & contains:'async fn'
...
**Description:** Cache file metadata for faster repeated queries
...
impl MetadataCache {
    fn get_or_fetch(&mut self, path: &Path) -> Result<&Metadata> {
        if let Some(entry) = self.entries.get(path) {
            let current_mtime = fs::metadata(path)?.modified()?;
            if entry.mtime == current_mtime {
                return Ok(&entry.metadata);  // Cache hit
            }
        }
        // Cache miss - fetch and store
        let metadata = fs::metadata(path)?;
        self.entries.insert(path.to_path_buf(), CacheEntry::new(metadata));
        Ok(&self.entries[path].metadata)
    }
}
```

**Performance Impact:**
- First query: ~5 seconds (10K files)
- Cached query: ~0.5 seconds (90% faster)

#### 4. Extended Output Formats

**Description:** XML, CSV, and custom template output

**XML Format:**
```xml
<?xml version="1.0"?>
<rdump>
  <file path="/src/main.rs" size="1234" modified="2024-01-15T10:30:00Z">
...
```csv
path,size,modified
/src/main.rs,1234,2024-01-15T10:30:00Z
```

**Custom Template:**
```bash
rdump "ext:rs" --template='{{path}}: {{size}} bytes'
```
...
2. **Language-Agnostic Predicates**
...
3. **Cross-Language Analysis**
   - `polyglot:` predicate for multi-language queries
   - Language detection improvements
   - Mixed-language file handling

### Version 0.4.0 - Analysis Features
...
1. **Dependency Analysis**
   - `depends:` - Files that depend on target
   - `dependents:` - Files that target depends on
...
3. **Pattern Matching**
   - `pattern:singleton` - Design pattern detection
   - `antipattern:godclass` - Anti-pattern detection
...
- Query suggestions from context
...
A collection of useful queries organized by task.
